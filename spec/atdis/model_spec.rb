...
describe ATDIS::Model do
  ...
  describe "#json_errors" do
    it "should return the json attribute with the errors" do
      ...
      expect(a.json_errors).to eq [[{bar: "Hello"}, [
  ...
  describe ".cast" do
    ...
    it "should cast arrays by casting each member" do
      expect(ATDIS::Model.cast([1, 2, 3], String)).to eq ["1", "2", "3"]
    end
  ...
  describe ".attribute_keys" do
    it do
      ...
      expect(ATDIS::Model.attribute_keys).to eq [:foo, :a, :info]
    end
  end
  ...
  describe ".partition_by_used" do
    ...
    it do
      allow(ATDIS::Model).to receive(:attribute_keys).and_return([:foo, :a])
