      ...
      RestClient.should_receive(:get).with("http://www.council.nsw.gov.au/atdis/1.0/applications.json").and_return(mock(:to_str => <<-EOF
{
  "response": [
    ...
    let(:applications_results) { ATDIS::ApplicationsResults.read(ATDIS::SeparatedURL.parse("http://www.council.nsw.gov.au/atdis/1.0/applications.json")) }
      ...
      RestClient.should_receive(:get).with("http://www.council.nsw.gov.au/atdis/1.0/applications.json?page=2").and_return(mock(:to_str => <<-EOF
{
  ...
  "pagination": {
    ...
    let(:applications_results) { ATDIS::ApplicationsResults.read(ATDIS::SeparatedURL.parse("http://www.council.nsw.gov.au/atdis/1.0/applications.json?page=2")) }
    ...
    it ".next" do
      ...
      ATDIS::ApplicationsResults.should_receive(:read).with(ATDIS::SeparatedURL.parse("http://www.council.nsw.gov.au/atdis/1.0/applications.json?page=3")).and_return(n)
