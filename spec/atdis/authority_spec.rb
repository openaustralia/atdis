...
describe ATDIS::Authority do
  describe "validations" do
    context "a valid ref" do
      let(:a) { ATDIS::Authority.new(ref: "http://www.council.nsw.gov.au/atdis/1.0")}
      it {a.should be_valid}
    end

    context "a valid ref with https" do
      let(:a) { ATDIS::Authority.new(ref: "https://www.council.nsw.gov.au/atdis/1.0")}
      it {a.should be_valid}
    end

    context "an invalid ref that isn't a url" do
      let(:a) { ATDIS::Authority.new(ref: "foobar")}
      it {
    ...
    context "an invalid ref because it doesn't end in atdis/1.0" do
      let(:a) { ATDIS::Authority.new(ref: "http://www.council.nsw.gov.au/foobar")}
      it {
        a.should_not be_valid
